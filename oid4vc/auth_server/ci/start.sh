...
if [ "$RUN_MODE" == "admin" ]; then
  ...
  uvicorn admin.main:app --host 0.0.0.0 --port 8000 --ssl-certfile /app/tls/tls.crt --ssl-keyfile /app/tls/tls.key
elif [ "$RUN_MODE" == "tenant" ]; then
  echo "Starting tenant server..."
  uvicorn tenant.main:app --host 0.0.0.0 --port 8001 --ssl-certfile /app/tls/tls.crt --ssl-keyfile /app/tls/tls.key
else
